...
<c:if test="${changeState == 1}">
	...
	<a href="changeState?id=<c:out value="${issue.id}"/>">Cambiar estado a EN CURSO</a>
	</c:if>
	<c:if test="${isOnCourse == 1}">
	<a href="changeState?id=<c:out value="${issue.id}"/>">Cambiar estado a ABIERTA</a>
	</c:if>
</c:if>
				...
				<tbody>
					<c:forEach items="${jobs}" var="job">
						<tr>
							...
							<td>
								<fmt:formatDate pattern="dd/MM/yyyy HH:mm:ss" value="${job.date}"/>
							</td>
							<td>
				...
				<tbody>
					<c:forEach items="${comments}" var="comment">
						<tr>
							...
							<td><fmt:formatDate pattern="dd/MM/yyyy" value="${comment.creationDate}" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
