...
public class DatabaseIssueDAO implements IssueDAO{
	...
	public void saveIssue(Issue issue) {
		...
		try{
			if(issue.getId() == -1) {
				...
				stmt = conn.prepareStatement("update issues set title=?, description=?, reporterId=?, assigneeId=?, projectId=?, estimatedTime=?, state=?, priority=?, resolution=? where id=?");
				stmt.setString(1, issue.getTitle());
				stmt.setString(2, issue.getDescription()); 
	...
	public List<Issue> getProjectIssuesWithFilter(Project project, issueFilter filter) {
		...
		String sql_st = (is_st)?new java.sql.Date(filter.getDate_st().getTime()).toString():""; 
		
		boolean is_et = filter != null && filter.getDate_et() != null;
		String sql_et = (is_et)?new java.sql.Date(filter.getDate_et().getTime()).toString():"";
		
		
		try {
			stmt = conn.prepareStatement("select * from issues where projectid = ? and (? is null or title like ?)"+
										"and (? is null or description like ?)"+
										"and (? = 0 or reporterid = ?)"+
										"and (? = 0 or assigneeid = ?)"+
										"and (? is null or state = ?)"+
										"and (? is null or resolution = ?)"+
										((is_st)?"and ( creationdate >= DATE '"+sql_st+"' )":"")+
										((is_et)?"and ( creationdate <= DATE '"+sql_et+"' )":"")
										);
			stmt.setInt(1, project.getId());
			...
			if (filter != null && filter.getReporter() != null && filter.getReporter().getId() >0 ){
			...
			if (filter != null && filter.getAsignee() != null && filter.getAsignee().getId() >0 ){
			...
			if (filter != null && filter.getState() != null && !filter.getState().toString().equals("ANY") ){
			...
			if (filter != null && filter.getResolution() != null && !filter.getResolution().toString().equals("ANY") ){
	...
	private Issue getNewIssue(ResultSet cur) {
		...
		try {
			...
			Project project = projectDAO.getProjectById(cur.getInt("projectId"));
			...
			issue = new Issue(cur.getString("title"), cur.getString("description"), estimatedTime, 
					cur.getDate("creationDate"), reporter, assignee, 
					State.valueOf(cur.getString("state")), resolution, 
					Priority.valueOf(cur.getString("priority")), project);
	...
	public void saveComment(Comment comment) {
		...
		try{
			stmt = conn.prepareStatement("insert into issues_comments (user_fk, issue_fk, comment, creation_date) values (?,?,?,?)");
	...
	public List<Comment> getIssueComments(Issue issue) {
		...
		try {
			stmt = conn.prepareStatement("select * from issues_comments where issue_fk = ?");
	...
	private Comment getNewComment(ResultSet cur) {
		...
		try {
			...
			comment = new Comment(cur.getDate("creation_date"),
					cur.getString("comment"), user, issue);
	...
	public void saveJob(Job job) {
		...
		Date date = new Date(job.getDate().getTime());
		
		int index = 0;
		
		try{
			stmt = conn.prepareStatement("insert into jobs (user_fk, issue_fk, duration, resolved_date, description) values (?,?,?,?,?)");
	...
	public List<Job> getIssueJobs(Issue issue) {
		...
		try {
			stmt = conn.prepareStatement("select * from jobs where issue_fk = ?");
			stmt.setInt(1, issue.getId());
			ResultSet cur = stmt.executeQuery();
	...
	private Job getNewJob(ResultSet cur) {
		...
		try {
			...
			job = new Job(cur.getInt("duration"), cur.getString("description"),
					user, cur.getDate("resolved_date"), issue);
